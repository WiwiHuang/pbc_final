...
def register_user(email, name, password, school, birthday, gender, height, weight, exercise,goal):
    ...
    # 插入新使用者資料
    cursor.execute("INSERT INTO users (email, name, password, school, birthday, gender, height, weight, exercise, goal) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
               (email, name, password, school, birthday, gender, height, weight, exercise, goal))
    conn.commit()  # 提交變更
    conn.close()   # 關閉資料庫連線
